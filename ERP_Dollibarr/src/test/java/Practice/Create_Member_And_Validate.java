...
public class Create_Member_And_Validate 
{
	public static void main(String[] args) throws IOException 
	{
		...
		WebElement firstNameVerify = driver.findElement(By.xpath("//span[text()='"+firstName+"']"));
		if(firstNameVerify.isDisplayed())
		{
			System.out.println(firstName +" is present in draft members");
		}
		else
		{
			System.out.println(firstName + " is not present in draft members");
		}
        firstNameVerify.click();
        MemberInfoPage mip=new MemberInfoPage(driver);
        mip.getValidateBtn().click();
        wlib.switchToWindowByTitle(driver, "Validate a member");
        driver.findElement(By.xpath("//button[text()='Yes']")).click();
        wlib.switchToParentFrame(driver);
        nmp.getValidatedMembersLink().click();
        WebElement fnVerifyInValidate = driver.findElement(By.xpath("//span[text()='"+firstName+"']"));
        if(fnVerifyInValidate.isDisplayed())
		{
			System.out.println(firstName +" is present in validated members");
		}
		else
		{
			System.out.println(firstName + " is not present in validated members");
		}
        driver.quit();
	}
}
